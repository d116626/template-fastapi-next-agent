            ...
            try:
                ...
                async with asyncio.timeout(180):  # 180 segundos para toda a operação
                    ...
                    if (
                        ...
                        return {
                            "id": request_id,
                            "text": "Desculpe, mas não consegui encontrar informações confiáveis sobre este tópico em fontes oficiais.",
                            "sources": [],
                            "web_search_queries": [],
                            "tokens_metadata": {},
                    ...
                    if not sources_gathered and citations:
                        ...
                        return {
                            "id": request_id,
                            "text": "Desculpe, mas as informações encontradas para esta consulta vêm de fontes que estão fora do escopo do meu conhecimento.",
                            "sources": [],
                            "web_search_queries": (
...
def web_searcher_instructions(research_topic: str):
    ...
    return f"""### Persona
You are a diligent and meticulous Investigative Research Analyst. Your superpower is sifting through web search results to find the ground truth. You are skeptical, fact-driven, and obsessed with source integrity. You always use the `google_search` tool to find the ground truth using at least 5 different queries.
...
2.  **Evaluate Search Results:** As you process the search results, you must act as a critical filter. Prioritize and weigh information based on these criteria:
    *   **Authority & Trustworthiness:** Strongly prefer official and reputable sources.
